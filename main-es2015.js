...
const routes = _services__WEBPACK_IMPORTED_MODULE_5__["Route"].withLayout([
    {
...
let StorageService = class StorageService {
    ...
    saveTask(title) {
        ...
        const tasksData = localStorage.getItem(this.TODO_LIST_KEY);
        const tasks = tasksData ? [...JSON.parse(tasksData), task] : [task];
    ...
    removeTask(task) {
        const tasksData = localStorage.getItem(this.TODO_LIST_KEY);
        const tasks = JSON.parse(tasksData).filter((item) => item.id != task.id);
        localStorage.setItem(this.TODO_LIST_KEY, JSON.stringify(tasks));
    }
    updateTask(task) {
        const tasksData = localStorage.getItem(this.TODO_LIST_KEY);
        const tasks = JSON.parse(tasksData).map((item) => {
            if (item.id === task.id) {
...
let InputTaskComponent = class InputTaskComponent {
    constructor() {
        this.title = '';
